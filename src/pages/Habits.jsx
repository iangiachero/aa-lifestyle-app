      ...
      <div className="page-safe-x pt-5">
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-[#6B6B6B] text-sm font-light">No habits yet</p>
            <p className="text-[#4B4B4B] text-xs mt-1">Tap + to build your first streak</p>
          </div>
        ) : (
                    ...
                    <div className="p-4">
                      <div className="flex items-start justify-between mb-3">
                        ...
                        <button
                          ...
                          className="w-6 h-6 flex items-center justify-center rounded-full hover:bg-[rgba(255,255,255,0.08)] transition-colors opacity-40 hover:opacity-80"
                        >
                          <Trash2 className="w-3.5 h-3.5 text-[#B8B8B8]" />
                        </button>
                      </div>
                      <p className="text-sm font-light text-[#F5F1E8] truncate mb-1.5">{habit.name}</p>
                      {(habit.streak || 0) > 0 && (
        ...
        {showModal && (
          ...
          >
            <motion.div
              ...
              className="w-full max-w-md bg-gradient-to-b from-[#111111] to-[#000000] rounded-t-3xl border-t border-x border-[rgba(201,169,98,0.3)] px-6 pt-6 pb-10"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-lg font-light text-[#F5F1E8]">New Habit</h2>
                <button
                  onClick={() => setShowModal(false)}
                  className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-[rgba(255,255,255,0.08)] transition-colors"
                >
                  <X className="w-4 h-4 text-[#B8B8B8]" />
                </button>
              </div>

              {/* Name */}
              <div className="mb-5">
                <label className="text-xs text-[#6B6B6B] uppercase tracking-widest mb-2 block">Name</label>
                <input
                  type="text"
                  placeholder="e.g. Morning run"
                  value={form.name}
                  onChange={(e) => setForm(f => ({ ...f, name: e.target.value }))}
                  className="w-full bg-[rgba(255,255,255,0.05)] border border-[rgba(201,169,98,0.2)] rounded-xl px-4 py-3 text-sm text-[#F5F1E8] placeholder-[#4B4B4B] outline-none focus:border-[rgba(201,169,98,0.5)] transition-colors"
                />
              </div>

              {/* Icon picker */}
              <div className="mb-5">
                <label className="text-xs text-[#6B6B6B] uppercase tracking-widest mb-2 block">Icon</label>
                    ...
                    <button
                      ...
                      className={`w-9 h-9 rounded-xl flex items-center justify-center text-lg transition-all border ${
                        form.icon === icon
                          ...
                          : 'border-[rgba(255,255,255,0.08)] bg-[rgba(255,255,255,0.04)] hover:border-[rgba(201,169,98,0.3)]'
                      }`}
                    >
                      {icon}
                    </button>
              ...
              {/* Color picker */}
              <div className="mb-7">
                <label className="text-xs text-[#6B6B6B] uppercase tracking-widest mb-2 block">Color</label>
