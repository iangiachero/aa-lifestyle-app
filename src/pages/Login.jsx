...
export default function Login() {
  ...
  return (
    <div
      ...
      style={{ background: 'var(--app-bg)', fontFamily: "'Inter', sans-serif" }}
    >
        ...
        <div className="rounded-2xl p-6 mb-6" style={{ background: 'rgba(0,0,0,0.5)', border: '1px solid rgba(201,169,98,0.2)', boxShadow: '0 8px 32px rgba(0,0,0,0.4)' }}>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button type="submit" disabled={loading} className="w-full py-4 rounded-xl text-lg font-medium flex items-center justify-center gap-2 transition-all duration-200 mt-2"
              style={{ fontFamily: "'Cormorant Garamond', serif", background: loading ? 'rgba(201,169,98,0.3)' : 'linear-gradient(135deg, #B8955A 0%, #C9A962 100%)', color: loading ? 'var(--app-wash-4)' : 'white', boxShadow: loading ? 'none' : '0 4px 20px rgba(184,149,90,0.3)', cursor: loading ? 'not-allowed' : 'pointer' }}>
              {loading ? <span className="inline-block w-5 h-5 border-2 border-[color:var(--app-wash-4)] border-t-white rounded-full animate-spin" /> : <>{mode === 'login' ? 'Sign In' : 'Create Account'}<ArrowRight size={18} /></>}
            </button>
          </form>
        </div>
