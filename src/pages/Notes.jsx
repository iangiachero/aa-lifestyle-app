      ...
      <div className="min-h-full pb-8 bg-[#000000]">
        <div className="px-6 pt-6 pb-4 border-b-2 border-[rgba(201,169,98,0.25)]">
          <div className="flex items-center justify-between mb-4">
            <button
              ...
              className="w-10 h-10 rounded-full bg-[#000000] border border-[rgba(201,169,98,0.3)] flex items-center justify-center hover:bg-[rgba(201,169,98,0.1)] transition-colors"
            >
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="w-10 h-10 rounded-full bg-[#000000] border border-red-400/30 hover:border-red-400 hover:scale-105 flex items-center justify-center transition-all"
              >
        ...
        <div className="px-6 pt-6 space-y-4">
          <input
            ...
            className="w-full text-2xl font-light bg-transparent border-none text-[#F5F1E8] placeholder-[#6B6B6B] focus:outline-none"
          />
          <textarea
            value={editingNote.content}
            onChange={(e) => handleAutoSave('content', e.target.value)}
            placeholder="Start writing..."
            className="w-full min-h-[500px] bg-transparent border-none text-[#F5F1E8] placeholder-[#6B6B6B] resize-none focus:outline-none leading-relaxed"
          />
        ...
        {showDeleteConfirm && (
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-6" onClick={() => setShowDeleteConfirm(false)}>
            <div className="w-full max-w-md bg-[#000000] rounded-2xl p-6 border-2 border-[rgba(201,169,98,0.3)]" onClick={(e) => e.stopPropagation()}>
              <h3 className="text-xl text-[#C9A962] font-light mb-2">Delete Note?</h3>
              <p className="text-sm text-[#B8B8B8] mb-6">Are you sure you want to delete "{selectedNote.title || 'Untitled Note'}"? This cannot be undone.</p>
              <div className="flex gap-3">
                <button onClick={() => setShowDeleteConfirm(false)} className="flex-1 py-3 bg-[#000000] border border-[rgba(201,169,98,0.3)] rounded-full text-sm text-[#B8B8B8] hover:bg-[rgba(201,169,98,0.1)] transition-colors">Cancel</button>
    ...
    <div className="min-h-full pb-8 bg-[#000000]">
      ...
      {notesLoading ? <NotesSkeleton /> : <div className="page-safe-x pt-4 space-y-4">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          <button onClick={handlePasswordsVault} className="w-full bg-[#000000] rounded-2xl p-4 border border-[rgba(201,169,98,0.3)] hover:border-[#C9A962] transition-all flex items-center gap-3">
            ...
            <div className="flex-1 text-left">
              ...
              <p className="text-xs text-[#B8B8B8]">Secure password storage</p>
            </div>
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-12 pr-4 py-3 bg-[#000000] border border-[rgba(201,169,98,0.3)] rounded-xl text-[#F5F1E8] placeholder-[#6B6B6B] focus:border-[#C9A962] focus:outline-none"
          />
        </div>
        ...
        <div className="space-y-3">
          {filteredNotes.length === 0 ? (
            <div className="text-center py-16">
              ...
              <div className="text-sm text-[#B8B8B8] mb-5">
            ...
            filteredNotes.map((note, idx) => (
              <motion.div key={note.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.03 }}>
                <button
                  ...
                  className="w-full bg-[#000000] rounded-2xl p-4 border border-[rgba(201,169,98,0.3)] hover:border-[#C9A962] hover:shadow-lg transition-all text-left"
                >
                  <div className="flex items-start gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between mb-2">
                        <h3 className="text-base text-[#F5F1E8] font-light line-clamp-1">{note.title || 'Untitled Note'}</h3>
                        {note.pinned && <Pin className="w-4 h-4 flex-shrink-0 ml-2 text-[#C9A962]" strokeWidth={1.5} />}
                      </div>
                      {note.content && (
                        <p className="text-sm text-[#B8B8B8] line-clamp-2 mb-2 leading-relaxed">{note.content}</p>
                      )}
                      <div className="text-xs text-[#6B6B6B]">
      ...
      {showPinModal && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-6" onClick={() => setShowPinModal(false)}>
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-md bg-[#000000] rounded-2xl p-6 border-2 border-[rgba(201,169,98,0.3)]" onClick={(e) => e.stopPropagation()}>
            <button onClick={() => setShowPinModal(false)} className="absolute top-4 right-4 w-8 h-8 flex items-center justify-center rounded-full hover:bg-[#000000] transition-colors">
              <X className="w-5 h-5 text-[#C9A962]" />
            </button>
            <div className="text-center mb-6">
              <div className="w-16 h-16 rounded-full bg-[#000000] flex items-center justify-center mx-auto mb-4">
                <Shield className="w-8 h-8 text-[#C9A962]" strokeWidth={1.5} />
              </div>
              <h3 className="text-xl text-[#C9A962] font-light mb-2">Create PIN</h3>
              <p className="text-sm text-[#B8B8B8]">Set a 4-6 digit PIN to secure your vault</p>
            </div>
            <div className="space-y-4 pb-32">
              <div>
                <label className="text-xs text-[#B8B8B8] uppercase mb-2 block">Create PIN (4-6 digits)</label>
                <input type="password" value={pin} onChange={(e) => setPin(e.target.value.replace(/\D/g, '').slice(0, 6))} placeholder="••••\" className="w-full px-4 py-3 bg-[#000000] border border-[rgba(201,169,98,0.3)] rounded-xl text-[#F5F1E8] placeholder-[#6B6B6B] focus:border-[#C9A962] focus:outline-none text-center text-2xl tracking-widest\" maxLength={6} />
              </div>
              <div>
